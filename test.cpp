...
class BiMean {
    ...
    void allocateArrs(){
        // cout << maxHeight << " " << maxVal << endl;
        GaussAry = new int[maxVal+1];
        // histGraph[maxHeight+1][maxVal+1];
        // GaussGraph[maxHeight+1][maxVal+1];
    ...
    void plotGraph(int *ary, char **graph, char symbol){
        ...
        */
        for(int i = 0; i<maxVal+1; i++){
            if(ary[i] >= 0){
                int point = (maxHeight+1)-ary[i]; //210 - point(1,210) ~ (1,0)
                // cout << i << " " << ary[i] << " " << point << " " << endl;
                for(int j=point; j<maxHeight; j++)
                    graph[j][i] = symbol;   //problem
                // cout << graph[point][i] << endl;
            }
        }
        cout<< "graph here";
        for(int i = 0; i<maxHeight; i++){
            for(int j=0; j<maxVal; j++){
                if((int)graph[i][j]==0) cout << " ";
                else cout << graph[i][j];
            }
            cout << endl;
        }
